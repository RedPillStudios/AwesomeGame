...
void Draw(int SpriteColumn1, int SpriteColumn2, int SpriteColumn3, int PosSprite, int Menuout, int frames, int frameExplosion, bool Mainmenu) {

	SDL_RenderClear(Main_Renderer);
	
	SDL_RenderCopy(Main_Renderer, LastBackground_Texture, NULL, &LastBackground);
	SDL_RenderCopy(Main_Renderer, LastBackground_Texture, NULL, &LastBackground2);
	...
	SDL_RenderCopy(Main_Renderer, Foreground_Texture, NULL, &Foreground);
	SDL_RenderCopy(Main_Renderer, Foreground_Texture, NULL, &Foreground2);
	...
	SDL_RenderCopy(Main_Renderer, Explosion_Texture, &Explosion_Clipping[frameExplosion / 4], &Explosion);
	if (!Mainmenu) {
	...
	if (Menuout) {
		SDL_RenderCopy(Main_Renderer, Background_Dead, NULL, NULL);
	...
	if(Mainmenu){
		SDL_RenderCopy(Main_Renderer, AwesomeGame_Texture, NULL, &AwesomeGame);
		SDL_RenderCopy(Main_Renderer, PressEnter_Texture, NULL, &Press);
		SDL_RenderCopy(Main_Renderer, Start_Texture, NULL, &Start); }
...
void SpriteSheetExplosion(SDL_Rect Sprite[]) {

	for (int i = 1; i < EXPLOSION_FRAMES; ++i) {
		if (i < 5) {
			Sprite[i].x = Sprite[i - 1].x + 113;
			Sprite[i].y = 0;
			Sprite[i].w = 113;
			Sprite[i].h = 114;
		}
